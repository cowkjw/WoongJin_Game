...
public class STileColorChange : MonoBehaviour
{
    ...
    private void Fill(Vector3Int cellPos)
    {
        ...
        // TODO : 색상 칠하기를 모두 마친 이후 몬스터들에게 자신의 위치가 플레이어 영역인지를 판단하게 한다.
    ...
    private void UpdateTileColorToMove(Vector3Int cellPos)
    {
        GetTileMap().SetTileFlags(cellPos, TileFlags.None);
        GetTileMap().SetColor(cellPos, new Color(_moveColor[0], _moveColor[1], _moveColor[2], _moveColor[3]));
    }

    private void UpdateTileColorToArea(Vector3Int cellPos)
    {
        GetTileMap().SetTileFlags(cellPos, TileFlags.None);
        GetTileMap().SetColor(cellPos, _areaColor);
    }
    ...
    private void ChangeMoveLIstTile(Vector3Int celPos)
    {
        ...
        foreach (var move in moveList)
        {
            ...
            UpdateTileColorToArea(new Vector3Int ( (int)move.x, (int)move.y, celPos.z ));
        }
    }
    ...
    private void SetJoyStick(bool value)
    {
        ...
        if (JoyStick != null)
        {
            JoyStick.GetComponent<SJoyStick>().SetCheck(value);
        }
    }
